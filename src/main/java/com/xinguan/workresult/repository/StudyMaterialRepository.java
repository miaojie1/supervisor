...
public interface StudyMaterialRepository extends JpaRepository<StudyMaterial, Long>, JpaSpecificationExecutor<StudyMaterial> {
    @Query(value = "select * from study_material where if(?1 !='',document_name like %?1%,1=1) and if(?2 !='',department_id = ?2,1=1) and if(?3 !='',study+material_category_id = ?3,1=1)",nativeQuery = true)
